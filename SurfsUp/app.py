...
def homePage():
    return(
        f"Welcome to Hawaii Climate Analysis API<br/><br/>"
        ...
        f"***Quick side note to run the last two routes *start* and *start/end* use the date format yyyy-mm-dd For example /api/v1.0/2016-08-28 ***"
    )

# precipitiation route that returns date an dprecipitation values
...
# stations route that displays the stations and thier information
...
# tobs route returing information on the most active station USC00519281
...
# start route to find min,max,avg temp of a given time
...
#start/end route to do the same as above but from a given time and end time
